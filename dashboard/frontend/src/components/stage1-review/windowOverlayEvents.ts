...
function markersFromAuditEvent(event: Stage1AuditEvent, bar?: ReviewCandleBar): VolumeLocationMarker[] {
...
/** Build every selected-class event in the current window — client-side source of truth for overlays. */
export function buildWindowOverlayEvents(params: {
  allEvents: Stage1AuditEvent[];
  context: Stage1ReviewContext;
  classSelection: Record<Stage1EventClass, boolean>;
  currentEvent: Stage1AuditEvent;
  ...
  return allEvents
    ...
    .map((event) => {
      ...
      const key = reviewKey(event.timestamp, event.event_class);
      const bar = barByIndex.get(barIndexInWindow);
      const markers = markerByKey.get(key) ?? markersFromAuditEvent(event, bar);
