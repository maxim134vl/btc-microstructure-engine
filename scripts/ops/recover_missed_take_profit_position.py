#!/usr/bin/env python3
"""Recover missed take-profit or restate a wrong close at TP price."""
...
from btc_ml.trading.intrabar_paper.take_profit_recovery import (
    RECOVERY_REASON_SYSTEM_BUG_TAKE_PROFIT,
    find_recovery_trade,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Recover missed TP or restate wrong close at TP")
    ...
    backup_root = epoch_root / "backups" / f"take_profit_recovery_{args.position_id}_{_utc_stamp()}"
    ...
    result = recover_missed_take_profit(
        ...
        recovery_reason=RECOVERY_REASON_SYSTEM_BUG_TAKE_PROFIT,
    )

    latest = latest_position_row(books, args.position_id)
    trade = find_recovery_trade(books, args.position_id)
    prior = find_strategy_trade_for_position(books, args.position_id)
    sleeves = SleeveLedger.load(epoch_root)
    m15 = sleeves.get("M15").to_dict() if sleeves is not None else None
    ...
    payload = {
        ...
        "effective_strategy_trade": prior,
        "m15_sleeve": m15,
