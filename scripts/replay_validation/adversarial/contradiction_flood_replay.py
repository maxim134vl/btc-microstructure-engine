"""Replay contradiction flood stress scenarios."""
...
def run(limit: int = 50, seed: int = 100) -> None:
    frame = load_probabilistic().tail(limit)
    print("CONTRADICTION FLOOD REPLAY")
    print("=" * 60)
    for index, (_, row) in enumerate(frame.tail(5).iterrows()):
        ...
        print(
            f"conflict {base.get('conflict_density', 0):.3f}"
            f" -> {stressed.get('conflict_density', 0):.3f}"
