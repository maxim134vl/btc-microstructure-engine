...
def _append_error_once(engine: ShadowEconomicCorrelationEngine, message: str) -> None:
    if not engine.errors or engine.errors[-1] != message:
        engine.errors.append(message)


def _build_engine() -> tuple[ShadowEconomicCorrelationEngine, dict[str, object]]:
    engine = ShadowEconomicCorrelationEngine(repo=REPO, strict_epoch=True)
    return engine, engine.write_health()
...
def _rollover_if_needed(
    ...
    replacement, health = _build_engine()
    print(
        json.dumps(
            {
                "status": "SHADOW_EQCORR_EPOCH_ROLLOVER",
                "pid": os.getpid(),
