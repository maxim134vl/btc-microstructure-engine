"""Append-only JSONL / JSON store confined to structural shadow directory."""
...
class ShadowStore:
    ...
    def __init__(self, root: Path | None = None, *, repo: Path | None = None) -> None:
        ...
        for name in ("policy_sleeves.json", "policy_manifest.json", "checkpoint.json", "health.json", "source_audit.json"):
            p = self.root / name
            if not p.exists():
                p.write_text("{}\n", encoding="utf-8")
    ...
    def table_sizes(self) -> dict[str, int]:
        return {
            table: self._jsonl(table).stat().st_size
    ...
    def read_inflight(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"INVALID_INFLIGHT_TRANSACTION:{exc}"
            ) from exc

        if not isinstance(raw, dict):
            raise RuntimeError(
                "INVALID_INFLIGHT_TRANSACTION:NOT_OBJECT"
            )
        ...
        if missing:
            raise RuntimeError(
                "INVALID_INFLIGHT_TRANSACTION:MISSING:"
                + ",".join(missing)
            )
        ...
        *,
        ...
        with self._lock:
            try:
                ...
                existing = self.read_inflight()
                raise RuntimeError(
                    "SHADOW_TRANSACTION_ALREADY_ACTIVE:"
                    f"{existing.get('kind')}:"
                    f"{existing.get('key')}:"
                    f"pid={existing.get('owner_pid')}"
                ) from exc
    ...
    def rollback_inflight(self) -> dict[str, Any]:
        marker = self.read_inflight()
        if not marker:
            return {"status": "NO_INFLIGHT_TRANSACTION"}

        sizes = marker.get("table_sizes")
        if not isinstance(sizes, dict):
            raise RuntimeError("INVALID_INFLIGHT_TABLE_SIZES")
        ...
        with self._lock:
            for table in self.TABLES:
                ...
                if target is None:
                    raise RuntimeError(
                        f"INFLIGHT_TABLE_SIZE_MISSING:{table}"
                    )

                target_i = int(target)
                if target_i < 0:
                    raise RuntimeError(
                        f"INFLIGHT_TABLE_SIZE_INVALID:"
                        f"{table}:{target_i}"
                    )
                ...
                if current < target_i:
                    raise RuntimeError(
                        "INFLIGHT_ROLLBACK_UNDERFLOW:"
                        f"{table}:{current}<{target_i}"
                    )

                with path.open("r+b") as fh:
                    fh.truncate(target_i)

            marker_path = assert_shadow_write_path(
                self.root / self.INFLIGHT_NAME,
                repo=self.repo,
            )
    ...
    def recover_inflight(
        ...
        *,
        ...
        base_generation = int(
            marker.get("base_generation") or 0
        )
        ...
        committed = (
            int(committed_generation) > base_generation
            and key in committed_keys
        )

        # A committed checkpoint is authoritative even when the
        # process died before removing the marker.
        if committed:
            self.clear_inflight()
            return {
                "status": "COMMITTED_MARKER_CLEARED",
                "kind": kind,
                "key": key,
                "owner_pid": owner_pid,
            }

        # Never let a duplicate runner roll back an active writer.
        if self._pid_is_alive(owner_pid):
            raise RuntimeError(
                "SHADOW_TRANSACTION_OWNER_ALIVE:"
                f"{kind}:{key}:pid={owner_pid}"
            )
