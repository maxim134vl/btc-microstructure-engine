"""Consume S4.1 command-bus rows into the Binance KZ executor. M15 only."""
...
from .constants import ALLOWED_TIMEFRAMES, CURSOR_SCHEMA_VERSION
from .executor import BinanceKzExecutor
from .ledger import utc_now


class BinanceKzCommandConsumer:
    def __init__(
        self,
        executor: BinanceKzExecutor,
        *,
    ...
    def poll(self) -> list[dict[str, Any]]:
        ...
        for tf in ALLOWED_TIMEFRAMES:
            ...
            for command in pending:
                ...
                if not command_id:
                    continue
                if str(command.get("timeframe") or "").upper() not in ALLOWED_TIMEFRAMES:
                    self._state.setdefault("processed_command_ids", []).append(command_id)
                    processed.add(command_id)
                    dirty = True
                    actions.append({"status": "IGNORED_NON_M15", "command_id": command_id})
                    continue
