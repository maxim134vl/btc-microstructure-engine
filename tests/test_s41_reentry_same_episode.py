"""Same directional episode may OPEN again on the next bar if parquet still wants it."""
...
if str(ROOT / "tests" / "trading") not in sys.path:
    sys.path.insert(0, str(ROOT / "tests" / "trading"))
...
from test_s41_live1b_chain_contract import _sources_for  # noqa: E402


def test_manager_reopens_same_episode_on_later_bar_when_flat(tmp_path, monkeypatch):
    monkeypatch.setattr(
        TimeframeManager,
    ...
    ctx = {"M15": "LONG_CONTEXT", "M30": "LONG_CONTEXT", "H1": "LONG_CONTEXT", "H4": "LONG_CONTEXT"}
